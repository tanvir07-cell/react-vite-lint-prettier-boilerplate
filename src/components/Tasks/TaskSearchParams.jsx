import React, { useState } from "react";
import CreateTaskModal from "./CreateTaskModal";
import DeleteAllTasksModal from "./DeleteAllTasksModal";
import { useTaskContext } from "../../context/TaskController";
import DeleteByParamsModal from "./DeleteByParamsModal";

const TaskSearchParams = () => {
  const [showModal, setShowModal] = useState(false);
  const [deleteAllModal, setDeleteAllModal] = useState(false);

  const { state, dispatch } = useTaskContext();
  const tasks = state.tasks;

  return (
    <>
      {showModal && (
        <CreateTaskModal onCloseModal={() => setShowModal(false)} />
      )}
      {deleteAllModal && (
        <DeleteAllTasksModal onCloseModal={() => setDeleteAllModal(false)} />
      )}

      <form
        onSubmit={(e) => {
          e.preventDefault();
          const formData = new FormData(e.target);
          dispatch({
            type: "SEARCH_TASK",
            payload: formData.get("search").toLowerCase(),
          });
        }}
      >
        <div className="flex">
          <div className="relative overflow-hidden rounded-lg text-gray-50 md:min-w-[380px] lg:min-w-[440px]">
            <input
              type="search"
              name="search"
              id="search-dropdown"
              className="z-20 block w-full bg-gray-800 px-4 py-2 pr-10 focus:outline-none"
              placeholder="Search Task"
              required
              onChange={(e) => {
                dispatch({
                  type: "SEARCH_TASK",
                  payload: e.target.value.toLowerCase(),
                });
              }}
            />

            <button
              type="submit"
              className="absolute right-2 top-0 h-full rounded-e-lg text-white md:right-4"
            >
              <svg
                className="h-4 w-4"
                aria-hidden="true"
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 20 20"
              >
                <path
                  stroke="currentColor"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="2"
                  d="m19 19-4-4m0-7A7 7 0 1 1 1 8a7 7 0 0 1 14 0Z"
                />
              </svg>
              <span className="sr-only">Search</span>
            </button>
          </div>
        </div>
      </form>
      <button
        onClick={() => setShowModal(!showModal)}
        className="rounded-md bg-blue-500 px-3.5 py-2.5 text-sm font-semibold"
      >
        Add Task
      </button>

      <button
        onClick={(e) => {
          e.preventDefault();
          setDeleteAllModal(!deleteAllModal);
        }}
        disabled={tasks.length === 0}
        style={
          tasks.length === 0 ? { cursor: "not-allowed" } : { cursor: "pointer" }
        }
        className="rounded-md bg-red-500 px-3.5 py-2.5 text-sm font-semibold"
      >
        Delete All
      </button>
    </>
  );
};

export default TaskSearchParams;
